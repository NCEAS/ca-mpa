...
CCFRP_envr_vars <- CCFRP_process %>%
    dplyr::select(c(SST="sst_monthly_anom",
...
kelp_fish <- kelp_fish_cen %>%
                  rowwise() %>%
                  dplyr::mutate(sum = sum(across(30:ncol(.), na.rm = T))) %>%
...
deep_reef <- deep_reef_cen %>%
                  rowwise() %>%
                  dplyr::mutate(sum = sum(across(31:ncol(.), na.rm = T))) %>%
                  filter(!(sum==0))%>% #remove rows containing only zeros
                  dplyr::select(-c(
...
###save group vars
#output_path <-  "/home/shares/ca-mpa/data/sync-data/monitoring/processed_data/community_climate_derived_data" 
...
#save(CCFRP_en, kelp_invalg_en, kelp_fish_en, deep_reef_en, rocky_en, file = file.path(output_path, "env_fit_scores.rda"))
...
```{r}
CCFRP_long <- CCFRP %>%
              pivot_longer(cols=25:ncol(.), names_to="species",
                           values_to = "counts")%>%
              filter(year >= 2007)
...
deep_reef_long <- deep_reef %>%
              pivot_longer(cols=26:ncol(.), names_to="species",
                           values_to = "counts")%>%
              filter(year >= 2007)
...
datalist <- list(CCFRP = CCFRP_join1, kelp_combined = kelp_combined_join1, 
        ...
        deep_reef = deep_reef_join1)

save(CCFRP_join1, kelp_combined_join1, 
                 kelp_swath_join1, kelp_upc_join1,
     kelp_fish_join1,
        deep_reef_join1, file= file.path("/home/shares/ca-mpa/data/sync-data/monitoring/processed_data/community_climate_derived_data","all_groups_mpa_level_means_long.rda"))

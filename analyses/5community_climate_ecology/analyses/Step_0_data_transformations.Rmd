...
#path_relative <- here::here("analyses","5community_climate_ecology","output")
#write.csv(kelp_stan, file.path(path_relative, "kelp_swath_upc_mpa_year.csv"), row.names = FALSE)
```
...
#output_path <-  "/home/shares/ca-mpa/data/sync-data/monitoring/processed_data/community_climate_derived_data/" #old

output_path <- here::here("analyses","5community_climate_ecology","output")
...
rocky_long <- rocky %>%
                #join air temp data
                left_join(rocky_Tair, by= c("site"="marine_site_name","year")) %>%
                dplyr::select(year, group, region3, region4, site, mpa_designation, affiliated_mpa, AT_anom, 
                              AT_abs, everything()) %>%
              #make longer
...
save(CCFRP_join1, kelp_combined_join1, 
     ...
     kelp_fish_join1,
        deep_reef_join1, 
     rocky_join1,
     file= file.path(here::here("analyses","5community_climate_ecology","output","all_groups_mpa_level_means_long.rda")))

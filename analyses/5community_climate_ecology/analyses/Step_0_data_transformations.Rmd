...
#drop higher order taxa that might include duplicates
kelp_stan <- kelp_stan %>% 
             select(!(c('apostichopus',
                        'anthopleura',
                        'asteroidea',
                        'haliotis',
                        'laminaria',
                       ...
                       'red_alga_with_cylindrical_branches_definition_used_in_2000',
                       'ucsc_2000_definition_of_other_red_included_all_red_algae_except_rhospp_bushy_leafy_crucor_and_erecor',
                       'ucsb_2000_definition_of_other_red_included_all_red_algae_except_rhospp_bushy_leafy_crucor_and_erecor',
                       'sargassum')))
...
CCFRP_process <- CCFRP_cen %>%
  ...
  select(!(c('mackerel_family_scombridae',
             'un_id_blue_rockfish',
             'unknown',
             'unknown_rockfish')))#drop species
...
deep_reef <- deep_reef_cen %>%
                  ...
                  dplyr::select(desig_state, everything())%>%
                  filter(mpa_defacto_designation=="smr" | mpa_defacto_designation=="ref")%>% #drop sum columns and non-species categories
                  select(!(c('sebastes_goodei',
                             'zaniolepis_frenata_or_latipinnis')))
#define grouping vars

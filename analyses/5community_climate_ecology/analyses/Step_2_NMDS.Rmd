...
kelp_upc <- as.data.frame(kelp_upc)              

kelp_upc[, 26:97][is.na(kelp_upc[, 26:97])] <- 0                  
...
#save envr vars 
#data_path <- "/home/shares/ca-mpa/data/sync-data/monitoring/processed_data/ecological_community_data/year_level_with_envr_vars"
#save(CCFRP_envr_vars,kelp_swath_envr_vars,kelp_upc_envr_vars,kelp_fish_envr_vars,
#  deep_reef_envr_vars,
...
kelp_upc_ord <- metaMDS(kelp_upc_distmat,previous.best=kelp_upc_ord, noshare=0.4, trymax = 999, parallel = num_cores)
  ...
  geom_path(data = df_ell, aes(NMDS1, NMDS2, colour=MHW, 
   ...
   geom_segment(aes(x = 0, y = 0, xend = (NMDS1), yend = (NMDS2)), 
     ...
     geom_text(data = en_coord_cont, aes(x = (NMDS1), y = (NMDS2)), colour = "black", 
       fontface = "bold", label = row.names(en_coord_cont), position=position_jitter(width=0.02,height=0.01), size=4) + 
  ggtitle("CCFRP")+
  scale_color_manual(values=c('#44b89d','#f56969','#4c78b5'))+
  theme(strip.text = element_text(size=18, face="bold"))+
  theme(plot.title = element_text(size=16, face="bold"))+
  labs(shape="site type",color='heatwave period') +
  theme(text = element_text(24))
...
#get NMDS scores
scrs <- as.data.frame(scores(kelp_swath_ord, display="site"))
en_coord_cont = as.data.frame(scores(kelp_swath_en, "vectors")) * ordiArrowMul(kelp_swath_en, fill=0.6)

scrs <- cbind(as.data.frame(scrs), desig_state=kelp_swath_group_vars$desig_state, year=kelp_swath_group_vars$year, region4=kelp_swath_group_vars$region4, mpa_designation=kelp_swath_group_vars$mpa_defacto_designation, MHW=kelp_swath_group_vars$MHW) #to facilitate computing centroids, add group var
...
kelp_swath<- ggplot(data=cent, aes(NMDS1, NMDS2)) +
  geom_point(aes(shape=mpa_designation, color=MHW), size=4) +                           #centroids
  geom_path(data = df_ell, aes(NMDS1, NMDS2, colour=MHW, 
                               group=group), size = 0.5, linetype=2, 
            )+           #line connecting groups (years)
...
scrs<- scores(kelp_upc_ord, display="sites")
...
en_coord_cont = as.data.frame(scores(kelp_upc_en, "vectors")) * ordiArrowMul(kelp_upc_en, fill=0.2)
...
kelp_upc<- ggplot(data=cent, aes(NMDS1, NMDS2)) +
...
figure<-ggarrange(CCFRP, kelp_fish, deep_reef,
          ...
          font.label=list(color="black",size=24)) + bgcolor('white')
...
#ggsave(here::here("analyses", "5community_climate_ecology", "figures", "community_structure_central.png"), figure, width = 14, units = "in", dpi = 300)
```

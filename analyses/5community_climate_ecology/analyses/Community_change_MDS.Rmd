...
year.CCFRP <- as.numeric(CCFRP_process$year)
region.CCFRP <- CCFRP_process$region4
mpa_designation.CCFRP <- CCFRP_process$mpa_designation
mhw.CCFRP <- CCFRP_process$MHW

CCFRP_ord_data <- CCFRP_process%>%
              dplyr::select(10:ncol(.))

CCFRP_distmat <- vegdist(CCFRP_ord_data, method = "bray") #generates a BC dissim matrix
...
kelp_swath <- kelp_swath_counts %>%
                  rowwise() %>%
                  mutate(sum = sum(10:ncol(.), na.rm = T)) %>%
                  filter(!(sum==0))%>%
                  dplyr::select(-sum) #remove rows containing only zeros

year.kelp_swath <- as.numeric(kelp_swath_counts$year)
region.kelp_swath <- kelp_swath_counts$region4
mpa_designation.kelp_swath <- kelp_swath_counts$mpa_defacto_designation
mhw.kelp_swath <- kelp_swath_counts$MHW
...
```{R}
CCFRP_ord <- metaMDS(distmat, distance="bray") #ordinates the dist matrix
kelp_swath_ord <- metaMDS(distmat, distance="bray") #ordinates the dist matrix

#export to .RData

path_aurora <- "/home/shares/ca-mpa/data/sync-data/processed_data/ecological_community_data" 
save(CCFRP_ord, kelp_swath_ord,file = file.path(path_aurora, "bray_nmds_scores"))

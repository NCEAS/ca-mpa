...
watch_nonconsump <- watch_nonconsump_orig %>% 
  ...
  rename(nonconsump_psurveys=psurveys, nonconsump_hr=activity_hr)

# MPA Watch consumptive
head(watch_consump_orig)
watch_consump <- watch_consump_orig %>% 
  select(mpa, psurveys, activity_hr) %>% 
  rename(consump_psurveys=psurveys, consump_hr=activity_hr)
...
ebird <- ebird_orig %>%
  ...
  rename(ebird_observers_n=n_observers, ebird_entries_n=n_observations)
  
# REEF data
head(reef_orig)
reef <- reef_orig %>% 
  select(mpa, nsurveys, nyrs) %>% 
  rename(reef_n=nsurveys, reef_nyr=nyrs)

# Permit data
head(permits_orig)
permits <- permits_orig %>% 
  select(mpa, npermits, nyears) %>% 
  rename(permits_n=npermits, permits_nyr=nyears)

# Citation data
head(citations_orig)
citations <- citations_orig %>% 
  select(mpa, ncitations, nyears) %>% 
  rename(citations_n=ncitations, citations_nyr=nyears)

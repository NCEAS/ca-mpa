...
#load data
biomass_dat <- read.csv(file.path(outdir, "targeted_nontargeted_fish_biomass.csv"))
...
biomass_data_mods <- left_join(biomass_dat, trait_select, by="affiliated_mpa") %>%
...
biomass_data_mods1 <- left_join(biomass_data_mods, 
                                fishing_pressure1,
                                by=c("affiliated_mpa" = "mpa"))
#check for unmatched mpas

fishing_anti <- anti_join(biomass_data_mods, 
                                   fishing_pressure1,
                                   by=c("affiliated_mpa" = "mpa"))


#export
#write.csv(biomass_data_mods1, file.path(outdir, "MPA_targeted_nontargeted_biomass_with_mods.csv"))

...
# Specify habitats
key_habitats = c("average_kelp", "coastal_marsh", 
                 "hard_0_30m", "hard_30_100m", "hard_100_200m", "hard_200_3000m",
                 "soft_0_30m", "soft_30_100m", "soft_100_200m", "soft_200_3000m",
                 "rocky_intertidal_and_cliff", "sandy_or_gravel_beaches",
                 "tidal_flats")
...
state_all <- data_core %>% 
  group_by(habitat) %>% 
  summarize(total_habitat_roms = sum(habitat_amount))

state_mpa <- data_mpa %>% 
  group_by(habitat) %>% 
  summarize(mpa_habitat_roms = sum(habitat_amount))

state <- full_join(state_all, state_mpa) 

  # mutate(pct_comp_all = total_habitat/sum(total_habitat)*100,
  #        pct_comp_mpa = mpa_habitat/sum(mpa_habitat)*100,
  #        pct_rep = mpa_habitat/total_habitat*100)

# Join with Attribute Data -----------------------------------------------------
all_state <- full_join(state, att_state)

all_state2 <- full_join(all_state, test)

...
data_sp <- data_kelp  %>% 
  ...
  mutate(age_at_survey = as.numeric(scale(age_at_survey)),
         ...
         hard_bottom_0_30m_500 =  as.numeric(scale(hard_bottom_0_30m_500))) 
```
...
data_terms <- data_sp %>% 
  dplyr::select(obs_id, age_at_survey, site_type, hard_bottom_0_30m_500) 
...
plot_partial_residuals <- function(model){
  ...
  if (identical(colnames(model_mat), names(fixed_coefs))){
    ...
    habitat <- ggplot(part_resid%>% filter(str_detect(term, "hard"))) +
      geom_smooth(aes(x = hard_bottom_0_30m_500, 
                      y = partial_residual, color = site_type),
                  show.legend = F) +
      labs(x = "Hard bottom 0-30m 500",
           y = "Partial residual", color = NULL) +
      scale_color_manual(values = c("#7e67f8", "#e5188b")) + 
      my_theme + 
      facet_wrap(~term)
...
```{r}
glm1 <- lmer(kg_per_m2 ~ hard_bottom_0_30m_500 * site_type + site_type * age_at_survey + 
...
```{r}
glm2 <- lmer(log_kg_per_m2 ~ hard_bottom_0_30m_500 * site_type + site_type * age_at_survey +
               (1|bioregion) + (1|affiliated_mpa) + (1|year), data = data_sp, REML = FALSE)
...
```{r}
glm3 <- lmer(kg_per_m2 ~ hard_bottom_0_30m_500 * site_type * age_at_survey + 
               (1|bioregion) + (1|affiliated_mpa) + (1|year), data = data_sp, REML = FALSE)
...
```{r}
glm4 <- lmer(log_kg_per_m2 ~ hard_bottom_0_30m_500 * site_type * age_at_survey + 
               (1|bioregion) + (1|affiliated_mpa) + (1|year), data = data_sp, REML = FALSE)
...
plot(predictorEffects(glm1, ~ hard_bottom_0_30m_500, partial.residuals = TRUE),
...
plot(predictorEffects(glm2, ~ hard_bottom_0_30m_500, partial.residuals = TRUE),
     axes = list(x = list(rotate = 25), y = list(lim = c(-1, 2))), id = list(n = 1),
     residuals.pch = 19,   
     residuals.cex = 0.2)
```
...
plot(predictorEffects(glm3, ~ hard_bottom_0_30m_500, partial.residuals = TRUE),
     axes = list(x = list(rotate = 25), y = list(lim = c(-1, 3))), id = list(n = 1),
     residuals.pch = 19,   
     residuals.cex = 0.2)
```
...
plot(predictorEffects(glm4, ~ hard_bottom_0_30m_500, partial.residuals = TRUE),
     axes = list(x = list(rotate = 25), y = list(lim = c(-1, 2))), id = list(n = 1),
     residuals.pch = 19,   
     residuals.cex = 0.2)
```
...
```{r}
gglm1 <- glmer(kg_per_m2_c ~ hard_bottom_0_30m_500 * site_type + site_type * age_at_survey + 
...
```{r}
gglm2 <- glmer(kg_per_m2_c  ~ hard_bottom_0_30m_500 * site_type * age_at_survey + 
                 (1|bioregion) + (1|affiliated_mpa) + (1|year),

...
process_buffer <- function(buffer, raster_set) {
  ...
  extracted %>%
    ...
    summarize(depth_mean = sum(depth * w_depth, na.rm = TRUE) / sum(w_depth, na.rm = T),
              ...
              tri_mean   = sum(tri * w_tri, na.rm = TRUE) / sum(w_tri, na.rm = TRUE), 
              relief     = abs(min(depth, na.rm = T) - max(depth, na.rm = T)),
              .groups = "drop") %>%
...
depth_df <- depth_30m %>% 
  ...
  dplyr::select(habitat, site, site_type, depth_mean, depth_sd, depth_cv, slope_mean, slope_sd, tri_mean, relief, buffer) 
...
grey_relief <- depth_df$relief[depth_df$site == "Greyhound Rock Reference" & depth_df$buffer == 100]
...
depth_df2 <- depth_df %>%
 mutate(depth_mean = ifelse(site == "Greyhound Rock Reference" & buffer %in% c(25, 50) & is.na(depth_mean), grey_mean, depth_mean),
        ...
        relief = ifelse(site == "Greyhound Rock Reference" & buffer %in% c(25, 50) & is.na(relief), grey_relief, relief))
...
process_anacapa <- function(buffer, ana_mpa, ana_ref, terrain_anacapa_mpa, terrain_anacapa_ref){
  ...
  extracted %>%
    ...
    summarize(depth_mean = sum(depth * w_depth, na.rm = TRUE) / sum(w_depth, na.rm = T),
              ...
              slope_sd   = sqrt(sum(w_slope * (slope - slope_mean)^2, na.rm = TRUE) / sum(w_slope, na.rm = T)), 
              tri_mean   = sum(tri * w_tri, na.rm = TRUE) / sum(w_tri, na.rm = TRUE), 
              relief     = abs(min(depth, na.rm = T) - max(depth, na.rm = T)), .groups = "drop") %>%
    mutate(buffer  = buffer)  %>% 
    mutate(depth_mean = depth_mean * -1) 
...
depth_df3 <- bind_rows(depth_df2, depth_ana_df) %>%
  dplyr::select(habitat, site, site_type, depth_mean, depth_sd, depth_cv, slope_mean, slope_sd, tri_mean, relief, buffer) %>% 
  pivot_longer(cols = c(depth_mean, depth_sd, depth_cv, slope_mean, slope_sd, tri_mean, relief), names_to = "variable", values_to = "value_m") %>% 
  # Since slope and TRI are calculated from 3x3 neighborhood, drop 25m and 50m scales:
  filter(!(variable %in% c("tri_mean", "slope_mean", "slope_sd", "depth_cv", "depth_sd") & buffer %in% c(25, 50))) %>% 
  ...
  select(habitat, site, site_type, starts_with("depth_mean"), starts_with("depth_sd"),  starts_with("depth_cv"), starts_with("tri"), starts_with("slope_mean"), starts_with("slope_sd"), starts_with("relief"))

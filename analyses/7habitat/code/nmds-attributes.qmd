...
ggplot(nmds_df, aes(x = NMDS1, y = NMDS2)) +
  ...
  #  Vector Labels
  geom_text(data = fit_att, 
            aes(x = NMDS1*0.75, y = NMDS2*0.75, label = att_label), size = 3) +
  # Formatting
  coord_fixed() +
...
ggplot(nmds_df, aes(x = NMDS1, y = NMDS2)) +
  ...
  # Vectors
  geom_segment(data = fit_att, 
               aes(x = 0, xend = NMDS1*0.6, y = 0, yend = NMDS2*0.6),
               arrow = arrow(length = unit(0.25, "cm")), col = "black") +
  #  Vector Labels
  geom_text(data = fit_att, 
            aes(x = NMDS1*0.65, y = NMDS2*0.65, label = att_label), size = 3) +
  scale_color_manual(values = c("purple", "orange", "green4", "blue")) +
  scale_x_continuous(limits = c(-0.55, 0.7), expand = c(0,0)) +
  # Formatting
  coord_fixed() +
  labs(color = "",
       shape = "") + 

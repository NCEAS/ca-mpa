...
habitat <- combined_df %>% 
  ...
  dplyr::select(-biotic) %>% 
  pivot_longer(c(soft_bottom:soft_bottom_biotic), names_to = "habitat_class", values_to = "area_m2", values_drop_na = T) %>% 
...
habitat2 <- habitat %>% 
  ...
  mutate_at(vars(hard_bottom_30_100m:soft_bottom_biotic_100_200m), ~ replace(., is.na(.), 0)) %>% 
  # Lengthen
  pivot_longer(cols = hard_bottom_30_100m:hard_bottom_biotic_30_100m, names_to = "habitat_depth", values_to = "area_m2") %>% 

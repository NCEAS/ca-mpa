...
att_data <- scale(att_data) %>% as.data.frame()
att_noest <- scale(att_noest) %>% as.data.frame()
```
...
ggplot(nmds_df, aes(x = NMDS1, y = NMDS2)) +
  ...
  geom_text(data = fit_att, 
            aes(x = NMDS1*10.5,
                y = NMDS2*10.5,
                label = att_short), size = 3) +
  geom_text(data = nmds_df %>% 
              filter(NMDS1 > 6 | NMDS2 > 5), aes(x = NMDS1, y = NMDS2, label = name),

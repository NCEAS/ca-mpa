...
# Calculate average depth of each MPA
...
# Read MPA polygons
mpas <- readRDS(file.path(out.dir, "CA_MPA_polygons.Rds"))
...
## Get depth values for each MPA
depth_mean <- aggregate(depth, by = mpas, mean, na.rm = TRUE)
...
## Add to MPA dataframe
data <- mpas %>% 
  select(name) %>% 
  mutate(mpa_mean_depth_m = depth_mean_sf$depth)

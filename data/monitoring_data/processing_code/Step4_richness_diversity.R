...
surf_effort <- surf %>% 
  # Individual hauls
  distinct(year, month, day, affiliated_mpa,mpa_defacto_class, mpa_defacto_designation, haul_number) %>% 
...
surf_diversity_build1 <- surf %>%
  ...
  #fix mpa class
  mutate(mpa_defacto_class = ifelse(affiliated_mpa == "ten mile smr","smr",mpa_defacto_class))
...
ccfrp_effort <- ccfrp %>%
  ...
  distinct(year, affiliated_mpa, mpa_defacto_class, mpa_defacto_designation, id_cell_per_trip) %>% 
  group_by(year, affiliated_mpa, mpa_defacto_class, mpa_defacto_designation) %>% 
...
ccfrp_diversity <- ccfrp_diversity_build1 %>% 
  ...
  filter(!(is.na(n_rep_mpa) | is.na(n_rep_ref))) # this drops the 2 years trinidad was sampled (ref only)
...
deep_effort <- deep %>% 
  distinct(year, affiliated_mpa, mpa_defacto_class, mpa_defacto_designation, line_id) %>% 
  group_by(year, affiliated_mpa, mpa_defacto_class, mpa_defacto_designation) %>% 
  summarize(n_rep = n()) %>% ungroup()
...
diversity_long <- diversity_full %>%
  pivot_longer(
    cols = c("shannon_unweighted_ref", "shannon_unweighted_mpa", 
             "richness_unweighted_ref", "richness_unweighted_mpa", 
             "shannon_weighted_ref", "shannon_weighted_mpa", 
             "richness_weighted_ref", "richness_weighted_mpa"),
    names_to = "factor",
    values_to = "value"
  )
...
diversity_long_scaled <- diversity_long %>%
  group_by(year, habitat, mpa_defacto_class, factor) %>%
  mutate(value = value + 0.1 * mean(value, na.rm = TRUE)) %>% 
  arrange(year, habitat, mpa_defacto_class, factor)

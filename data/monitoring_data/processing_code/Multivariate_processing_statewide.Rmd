...
#Export year level
#path_aurora <- "/home/shares/ca-mpa/data/sync-data/monitoring/processed_data/community_climate_derived_data" 
#write.csv(kelp_swath_mpa_year, file.path(path_aurora, "kelp_swath_mpa_year_statewide.csv"), row.names = FALSE)
...
#Export
path_aurora <- "/home/shares/ca-mpa/data/sync-data/monitoring/processed_data/community_climate_derived_data" 
write.csv(kelp_upc_mpa_year,file.path(path_aurora, "kelp_upc_mpa_year_statewide.csv"), row.names = FALSE)
...
#drop species that were never observed 
CCFRP_zero_drop <- CCFRP_CPUE  %>%
                    select(!(where(~ any(. != 0))))

CCFRP_cen <- CCFRP_CPUE %>% 
                    select(where(~ any(. != 0)))

#Export
path_aurora <- "/home/shares/ca-mpa/data/sync-data/monitoring/processed_data/community_climate_derived_data" 
write.csv(CCFRP_cen,file.path(path_aurora, "CCFRP_statewide.csv"), row.names = FALSE)
...
deep_reef_counts_wide_drop <- deep_reef_counts_wide %>% 
                    select(!(where(~ any(. != 0))))

deep_reef_counts_wide1 <- deep_reef_counts_wide %>% 
                    select(where(~ any(. != 0)))
...
#Export
path_aurora <- "/home/shares/ca-mpa/data/sync-data/monitoring/processed_data/community_climate_derived_data" 
write.csv(deep_reef_merge,file.path(path_aurora, "deep_reef_statewide.csv"), row.names = FALSE)

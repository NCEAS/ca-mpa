...
#Old
#envr_dat1 <- readRDS("/home/shares/ca-mpa/data/sync-data/environmental/processed/envr_anomalies_at_mpas.Rds")

#load data and get mpa-year level means
envr_dat1 <- readRDS("/home/shares/ca-mpa/data/sync-data/environmental/processed/envr_anomalies_at_mpas_new.Rds") %>%
            ...
            "bottomT_annual_baseline", "bottomT_monthly_baseline", "bottomT_annual_obs", "bottomT_monthly_obs", "bottomT_monthly_anom",
...
rocky_count_envr <- left_join(rocky_counts_raw, rocky_envr_vars, by=c("year","site"))%>%
  ...
  mutate(row_sum = rowSums(.[24:65]))

#convert to proportion after substrate drop (rock sand tar)

rocky_count_prop <- rocky_count_envr %>%
                    rowwise() %>%
                    mutate(across(24:64)/row_sum)%>%
                    select(!(row_sum))


#export
#path_aurora <- "/home/shares/ca-mpa/data/sync-data/monitoring/processed_data/community_climate_derived_data" 
#write.csv(rocky_count_prop, file.path(path_aurora, "rocky_mpa_year.csv"), row.names = FALSE)
...
prop <- rocky_count_envr[,24:66]

group <- rocky_count_envr[,1:2]

rocky_distmat <- 
    vegan::vegdist(prop, method = "bray", na.rm=T)

rocky_10_dist <- meandist(rocky_distmat, grouping = group$year)
...
betadisp <- betadisper(rocky_distmat, group = group$year, type="centroid")

beta_clust <- hclust(as.dist(betadisp$centroids),
                     method="average")

plot(beta_clust)


rocky_clust2 <- hclust(rocky_distmat)

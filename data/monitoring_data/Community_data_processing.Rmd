...
CCFRP_mpa_year_step1 <- CCFRP_CPUE %>%
...
CCFRP_mpa_year <- left_join(CCFRP_mpa_year_step1, envr_dat, by=c("year","group",
                  ...
                  dplyr::select(!(c("total", "mpa_class.y","mpa_designation.y")))%>%
                  dplyr::select(1:8,102:117,9:101)
...
kelp_fish_mpa_year_step1 <- kelp_fish_counts %>% 
...
kelp_fish_mpa_year <- left_join(kelp_fish_mpa_year_step1, envr_dat, by=c("year","group",
                                                       ...
                                                       "affiliated_mpa"="mpa_name"))%>%
                  dplyr::select(1:7,151:169,8:150) %>%
                  dplyr::select(!c("mpa_class","mpa_designation")) %>%
                  dplyr::select(group, everything()) %>%
                  mutate(group=recode(group,kelp="kelp_fish"))
...
kelp_swath_mpa_year_step1 <- kelp_swath_counts %>%
...
kelp_swath_mpa_year <- left_join(kelp_swath_mpa_year_step1, envr_dat, 
                                      by=c("year","group",
                                                       ...
                                                       "mpa_defacto_designation",
                                                       "affiliated_mpa"="mpa_name"))%>%
                  dplyr::select(1:7,176:194,8:175) %>%
                  dplyr::select(!c("mpa_class","mpa_designation")) %>%
                  dplyr::select(group, everything()) %>%
                  mutate(group=recode(group,kelp="kelp_swath"))
...
deep_reef_counts_envr <- left_join(deep_reef_counts_wide, envr_dat, 
                                  by=c("year","group",
                                                    "region3","region4",
                                                    "mpa_class"="mpa_class",

...
# Read data
data <- readRDS(file=file.path(outdir, "ecotrust_survey_data_comm.Rds"))
...
plot_data <- function(q){
  ...
  fdata <- data %>% 
    ...
    filter(question==q) %>% 
    # Summarize
    group_by(region, port_complex, response_id, response) %>% 
    summarize(n=n()) %>% 
    ungroup() %>% 
  ...
  # Export
  figname <- paste0("FigX_comm_", gsub(" |\\/", "_", tolower(q)), ".png")

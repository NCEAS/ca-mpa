...
data_s <- data_s_orig %>% 
  ...
  mutate(substrate1=recode(substrate1, "unknown"="Unknown"),
         ...
         region="Southern") %>% 
  # Arrange
  dplyr::select(region, substrate1, depth, geometry) %>% 
  dplyr::rename(substrate=substrate1)
...
data_c <- data_c_orig %>% 
  ...
  # Arrange
  dplyr::select(region, substrate, depth, geometry)
...
#sf::st_write(data_sf, dsn=file.path(outdir, "CA_substrate_polygon_stanford.shp"))

...
#Richness plot that uses year as independent reps (see n_MPA n_REF)
```{r}
...
region.yr.means$indicator<- recode_factor(region.means$indicator, Richness="richness")
...
#fit random effects model
res <- rma(yi,vi,method="REML", level=95, data=dat,  slab=paste(group)) 
...
### add text for the subgroups
text(-8, c(18,12,6), pos=4, c("North",
                               "Central",
                               "South"))
...
### fit random-effects model in the three subgroups
res.n <- rma(yi, vi, subset=(mlpa_region=="North"), level=95, data=dat, slab=paste(group, mlpa_region))
res.c <- rma(yi, vi, subset=(mlpa_region=="Central"), level=95, data=dat, slab=paste(group, mlpa_region))
res.s <- rma(yi, vi, subset=(mlpa_region=="South"), level=95, data=dat, slab=paste(group, mlpa_region))

### add summary polygons for the three subgroups
#addpoly(res.n, row=11, mlab=mlabfun("RE Model for Subgroup", res.n))
addpoly(res.n, row=13, mlab=mlabfun("RE Model for Subgroup", y=res.n))
addpoly(res.c, row= 7, mlab=mlabfun("RE Model for Subgroup", y=res.c))
addpoly(res.s, row= 1, mlab=mlabfun("RE Model for Subgroup", y=res.s))
...
```{r}
meta.data$mpa_designation<- recode_factor(meta.data$mpa_designation, reference="REF", SC="MPA", SMR_SCMA="MPA", SMR_SMCA="MPA", SMCA="MPA",SMR="MPA")
MPA.means<- meta.data%>%
  ...
  summarize(MPA_mean = mean(mean,na.rm=TRUE), 
            sd=mean(sd), # standard deviation of across MPAs where indicator was observed/recorded
            n=n())%>%#no. MPA 'sites' where year is unit of rep
            pivot_wider(names_from = mpa_designation, #convert to wide format for {metafor}
...
MPA.means <- MPA.means %>%
        pivot_wider(names_from = survey_period, #convert to wide format for {metafor}
            values_from = c(MPA_mean_REF, MPA_mean_MPA, sd_REF,sd_MPA, n_REF, n_MPA))
...
Response ratios plotted like forest plot
...
## Manually Calculate T Score, P Value, and logRR
...
#dat <- escalc(measure="SMD", m1i=mean_MPA, m2i=mean_REF, sd1i=sd_MPA, sd2i=sd_REF, n1i=n_MPA, n2i=n_REF, data=means_format, slab=paste(group)) 



#means by MPA

mu_site <- means_format %>% 
  mutate(mean_diff = mean_MPA-mean_REF,
         ...
         RR        = log(mean_MPA/mean_REF))

 
mu_site[is.finite(mu_site$RR) & is.finite(mu_site$RR), ]     #drop NA and Inf for now...



#means by region

mu_region <- as.data.frame(means %>% 
  mutate(mpa_designation = if_else(mpa_designation %in% c("reference", "REF"), "REF", "MPA")) %>% 
  group_by( group, mlpa_region, mpa_designation, variable, indicator) %>%
  summarize(yr.mean = mean(mean,na.rm=TRUE), 
                        sd=sd(mean), # standard deviation across MPAs AND YEARS where indicator was observed/recorded. 
                        n=n()) %>%
  pivot_wider(id_cols = c(group, mlpa_region, mpa_designation, variable, indicator),
              names_from = mpa_designation,
              values_from = c(yr.mean, sd, n)) %>% 
  mutate(n_total   = n_MPA+n_REF,
         mean_diff = yr.mean_MPA-yr.mean_REF,
         se        = sqrt((sd_MPA^2/n_MPA)+(sd_REF^2/n_REF)),
         t_score   = mean_diff/se,
         df        = n_MPA + n_REF - 2,
         p_value   = pt(t_score, df, lower.tail = FALSE),
         sig       = if_else(p_value < 0.05, 1, 0),
         cohens_d  = abs(mean_diff)/(sqrt(((sd_MPA^2*(n_MPA-1))+(sd_REF^2*(n_REF-1)))/df)),
         RR     = yr.mean_MPA/yr.mean_REF))
mu_region <- mu_region[is.finite(mu_region$RR) & is.finite(mu_region$RR), ]  

mu_region <- mu_region %>%
        filter(se>0)%>%
        mutate(
          lower.CI  = (RR - (qt(0.975, df)*n_total/sqrt(n_total))),
          upper.CI  = (RR + (qt(0.975, df)*n_total/sqrt(n_total))),
          )
...
mu_region$indicator<- recode_factor(mu_region$indicator, Diversity="diversity")
mu_region$indicator<- recode_factor(mu_region$indicator, shannon="diversity")
mu_region$indicator<- recode_factor(mu_region$indicator, Richness="richness")


mu_region.H <- mu_region %>%
  filter(variable=="All Fish" | 
         variable=="INVALG" |
         variable=="all species",
         indicator=='diversity')

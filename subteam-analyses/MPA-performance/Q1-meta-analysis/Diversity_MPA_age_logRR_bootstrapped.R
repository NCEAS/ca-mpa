...
means_wide <- means %>% 
  ...
  pivot_wider(id_cols = c(affiliated_mpa, group, mlpa_region, variable, indicator, mpa_age),
...
## Check the dataset if the means column has zeros
## If yes, we will need to add a constant to all of the means to avoid undefined ratios (constant = 10% of the grand mean)
# Calculate the grand mean for each indicator
m <- mean((drop_data.2$mean_smr+drop_data.2$mean_ref), na.rm = T)

# Calculate 10% of the grand mean
m.1 <- 0.1*m

# Add the constant to all of the means
drop_data.2$mean_smr <- m.1+drop_data.2$mean_smr
drop_data.2$mean_ref <- m.1+drop_data.2$mean_ref
...
#means.final <- means.final[!is.infinite(means.final$logRR),]        #not needed since we added 10% of grand mean 
...
CCFRP <- means.final %>%
  filter(group=='ccfrp')            #start with CCFRP to get loop functioning
...
# Sort the age in an ascending order to ensure the loop will start from the earliest year
set_data <- set_data[order(set_data$mpa_age),]
...
for (i in 1:length(uniq_region)){
  ...
  # Identify individual mpa_age within each region
  uniq_age <- unique(unlist(data_1$mpa_age))
  
  # Run the loop for age within each region
  for (i in 1:length(uniq_age)){
    data_2 <- subset(data_1, mpa_age == uniq_age[i])
    ...
    for (i in 1:length(uniq_var)){
      ...
      size <- function (x) summarySE(data_3, measurevar = "mean", groupvars = c("mlpa_region", "mpa_designation", "mpa_age", "variable"), na.rm = T)
      sample.size <- size(data_3)
      s_mpa <- sample.size$N[sample.size$mpa_designation == "smr"]
      ...
      age_out <- data_3$year[1]         # Create a output
      age <- data.frame(c(age, age_out))
...
resampled_1<- set_data[order(set_data$mlpa_region, set_data$mpa_age, set_data$mpa_designation),]
resampled_1 <- set_data%>%
  dplyr::group_by(group,mlpa_region,mpa_age,variable,indicator)%>%
...
CCFRP.resampled_RR_CI <- cbind(resampled_1, CI_5, CI_95, age, n_mpa, n_ref, region) 

# Reset row names
rownames(CCFRP.resampled_RR_CI) <- NULL
...
CCFRP.resampled_RR_CI$age <- as.numeric(CCFRP.resampled_RR_CI$age)

ggplot(CCFRP.resampled_RR_CI, aes(x=mpa_age, y=logRR))+
...
kf_swath <- means.final %>%
  filter(group=='kelp forest-swath')            #start with CCFRP to get loop functioning
...
# Combine true means, null confidence intervals and year into one data frame
KF_swath.resampled_RR_CI <- cbind(resampled_1, CI_5, CI_95, age, n_mpa, n_ref, region) 

# Reset row names
rownames(KF_swath.resampled_RR_CI) <- NULL
...
ggplot(KF_swath.resampled_RR_CI, aes(x=mpa_age, y=logRR))+
  geom_hline(yintercept=0, linetype="dashed", color = "gray", size = 0.5) +
  geom_ribbon(aes(ymax=eCI_95, ymin=eCI_5), group=1, alpha=0.2, fill = "#BC1605") +
  geom_point(shape=19, size=3, color = "#BC1605") +
  geom_smooth(method='lm', se = T, formula=y~x, linetype='solid', size=0.5) +
...
kf_upc <- means.final %>%
  filter(group=='kelp forest-upc')            #start with CCFRP to get loop functioning
...
# Combine true means, null confidence intervals and year into one data frame
KF_upc.resampled_RR_CI <- cbind(resampled_1, CI_5, CI_95, age, n_mpa, n_ref, region) 

# Reset row names
rownames(KF_upc.resampled_RR_CI) <- NULL
...
ggplot(KF_upc.resampled_RR_CI, aes(x=mpa_age, y=logRR))+
  geom_hline(yintercept=0, linetype="dashed", color = "gray", size = 0.5) +
  geom_ribbon(aes(ymax=eCI_95, ymin=eCI_5), group=1, alpha=0.2, fill = "#BC1605") +
  geom_point(shape=19, size=3, color = "#BC1605") +
  geom_smooth(method='lm', se = T, formula=y~x, linetype='solid', size=0.5) +
...
kf_fish <- means.final %>%
  filter(group=='kelp forest-fish')            #start with CCFRP to get loop functioning
...
# Combine true means, null confidence intervals and year into one data frame
KF_fish.resampled_RR_CI <- cbind(resampled_1, CI_5, CI_95, age, n_mpa, n_ref, region) 

# Reset row names
rownames(KF_fish.resampled_RR_CI) <- NULL
...
ggplot(KF_fish.resampled_RR_CI, aes(x=mpa_age, y=logRR))+
  geom_hline(yintercept=0, linetype="dashed", color = "gray", size = 0.5) +
  geom_ribbon(aes(ymax=eCI_95, ymin=eCI_5), group=1, alpha=0.2, fill = "#BC1605") +
  geom_point(shape=19, size=3, color = "#BC1605") +
  geom_smooth(method='lm', se = T, formula=y~x, linetype='solid', size=0.5) +
...
deep_reef <- means.final %>%
  filter(group=='deep-reef')  #start with CCFRP to get loop functioning
...
# Combine true means, null confidence intervals and year into one data frame
deep_reef.resampled_RR_CI <- cbind(resampled_1, CI_5, CI_95, age, n_mpa, n_ref, region) 

# Reset row names
rownames(deep_reef.resampled_RR_CI) <- NULL
...
ggplot(deep_reef.resampled_RR_CI, aes(x=mpa_age, y=logRR))+
  geom_hline(yintercept=0, linetype="dashed", color = "gray", size = 0.5) +
  geom_ribbon(aes(ymax=eCI_95, ymin=eCI_5), group=1, alpha=0.2, fill = "#BC1605") +
  geom_point(shape=19, size=3, color = "#BC1605") +
  geom_smooth(method='lm', se = T, formula=y~x, linetype='solid', size=0.5) +
  ...
  labs(y=expression(bold(Log(Diversity[MPA]/Diversity[REF])))) +
  labs(colour = 'Difference', linetype = 'Slope') +
  facet_wrap(~mlpa_region, ncol=1,scales = "fixed")+
  scale_x_continuous(breaks = c(-10:21))+
  theme_classic() 
...
resampled_full <- rbind(CCFRP.resampled_RR_CI,KF_swath.resampled_RR_CI, KF_upc.resampled_RR_CI, KF_fish.resampled_RR_CI, deep_reef.resampled_RR_CI)

resampled <- resampled_full %>%
  filter(mpa_age >= -5)

ggplot(resampled, aes(x=mpa_age, y=logRR))+
  geom_hline(yintercept=0, linetype="dashed", color = "gray", size = 0.5) +
  geom_ribbon(aes(ymax=eCI_95, ymin=eCI_5), group=1, alpha=0.2, fill = "#BC1605") +
  geom_point(shape=19, size=3, color = "#BC1605") +
  geom_smooth(method='lm', se = T, formula=y~x, linetype='solid', size=0.5) +
  ...
  scale_color_manual(values=c("#000000", "#A4A4A4"), drop = FALSE) +
  scale_linetype_manual(values=c("solid","twodash"), drop = FALSE) +
  labs(x="MPA age", y=expression(bold(Log(Diversity[MPA]/Diversity[REF])))) +
  labs(colour = 'Difference', linetype = 'Slope') +
...
means_age <- means %>% 
  filter(variable=="all species"| variable=="all fish"| variable=="invalg",
         indicator == "diversity",
         mpa_class=="smr"| mpa_class=="ref",
         group=="ccfrp"|group=="deep-reef"|group=="kelp forest-fish"|group=="kelp forest-swath"|group=="kelp forest-upc",
         mpa_age >= -5)%>%
  group_by(group,mlpa_region, mpa_designation,variable, mpa_age, indicator)%>%
  dplyr::summarize(grand.mean = mean(mean,na.rm=TRUE), 
                   sd=sd(mean), # standard deviation of across MPAs where indicator was observed/recorded
                   n=n())

means_age$mlpa_region <- factor(means_age$mlpa_region, levels = c("north", "central","south"))



ggplot(means_age, aes(x=mpa_age, y=grand.mean, color=mpa_designation))+
...
means_year <- means %>% 
  filter(variable=="all species"| variable=="all fish"| variable=="invalg",
         ...
         group=="ccfrp"|group=="deep-reef"|group=="kelp forest-fish"|group=="kelp forest-swath"|group=="kelp forest-upc",
         mpa_age >= -5)%>%
  group_by(group,mlpa_region, mpa_designation,variable, year, indicator)%>%
  dplyr::summarize(grand.mean = mean(mean,na.rm=TRUE), 
                   sd=sd(mean), # standard deviation of across MPAs where indicator was observed/recorded
                   n=n())

means_year$mlpa_region <- factor(means_year$mlpa_region, levels = c("north", "central","south"))



ggplot(means_year, aes(x=year, y=grand.mean, color=mpa_designation))+
  geom_point(shape=19, size=3) +
  geom_smooth(method='lm', se = T, formula=y~x, linetype='solid', size=0.5) +
  stat_poly_eq(formula = y ~ x, 
               aes(label = paste(..rr.label.., ..p.value.label.., sep = "*`,`~")), 
